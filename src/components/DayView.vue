...
function updateClock() {
  ...
  currentTime.value = `${day} - ${now.toLocaleTimeString()}`;
}

onMounted(async () => {
  try {
    const dir = (await appDataDir()) + 'my-app-data';
    dailyTotals.value = await invoke<Record<string, number>>('get_daily_summary_csv', { directory: dir });
  } catch {
    dailyTotals.value = {};
  }
  updateClock();
...
const maxTotal = computed(() => Math.max(...Object.values(dailyTotals.value), 0));
    ...
    <div v-for="src of ['U','E','S','F']" :key="src" class="bar-row">
      ...
      <div class="bar-bg">
        <div
          class="bar-fill"
          :style="{
            width: maxTotal > 0 ? ((dailyTotals[src] || 0) * 100 / maxTotal) + '%' : '0%',
            backgroundColor: colorMap[src]
          }"/>
          <p>aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</p>
      </div>
      <span class="label-right">{{ formatTime(dailyTotals[src] || 0) }}</span>
    </div>
